...
If the SLO is "99% of all requests will take less than 20ms in a given month (Latency)", then SLI can be expressed as the "percentage of requests successfully retrieving before 20ms over the past month".
...
traffic: Given a counter, the query calculate per-second average requests over the last hour for the past 4 weeks, with the resolution of : `rate(http_requests_total[1h])[4w:10m]`
### latency: 
//average request duration during the last 5 minutes:
  `rate(prometheus_http_request_duration_seconds_sum[5m])/rate(prometheus_http_request_duration_seconds_count[5m])`
more metrics:
`sum(rate(prometheus_http_request_duration_seconds_bucket{le="0.02"}[5m])) by (job)/  sum(rate(prometheus_http_request_duration_seconds_count[5m])) by (job)`
